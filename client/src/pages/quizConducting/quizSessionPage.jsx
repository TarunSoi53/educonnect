...
const QuizSessionPage = () => {
  ...
  const studentId = 'USER_ID_PLACEHOLDER'; // Replace with actual user ID logic (e.g., from context)

  const [questions, setQuestions] = useState([]);
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [selectedAnswers, setSelectedAnswers] = useState([]); // Stores the index of the selected option
  ...
  const fetchQuestions = async (quizId) => {
    try {
      ...
      setSelectedAnswers(new Array(response.data.quizQuestions.length).fill(undefined));
      setQuizStarted(true);
    } catch (error) {
  ...
  const handleSubmitQuiz = async () => {
    if (submissionLoading || quizCompleted || !quizStarted) return;
  
    setSubmissionLoading(true);
    
    // Convert selected answer indices to letters (a, b, c, d)
    const submittedQuizAnswers = questions.map((question, index) => {
      const selectedOptionIndex = selectedAnswers[index];
      const selectedAnswerLetter = selectedOptionIndex !== undefined 
        ? String.fromCharCode(97 + selectedOptionIndex) // 0 -> 'a', 1 -> 'b', etc.
        : undefined; // or null if backend expects it
  
      return {
        questionId: question._id,
        selectedAnswer: selectedAnswerLetter, // Send as letter
      };
    });
    ...
    try {
      const response = await api.post('/api/quizzes/submitQuiz', {
        quizId: quizId,
        quizAnswers: submittedQuizAnswers,
        timeTaken: timeTaken,
      });
        ...
        <div className="flex-1 bg-white rounded-lg p-6 shadow-lg border border-gray-200">
          {quizCompleted && submissionResult ? (
            <div>
                    ...
                    <ul className="mt-2 space-y-1">
                      {questions[index]?.options.map((option, optionIndex) => (
                        <li
                          key={optionIndex}
                          className={`${
                            String.fromCharCode(97 + optionIndex) === result.selectedAnswer?.toLowerCase()
                              ? result.isCorrect
                                ? 'text-blue-500 font-semibold'
                                : 'text-red-500 font-semibold line-through'
                              : String.fromCharCode(97 + questions[index]?.correctAnswer?.toString())?.toLowerCase() === optionIndex.toString()
                              ? 'text-green-500 font-semibold'
                              : 'text-gray-700'
                          }`}
                        >
                          {option}
                          {String.fromCharCode(97 + optionIndex) === result.correctAnswer?.toLowerCase() && (
                            <span className="text-green-500"> (Correct Answer)</span>
                          )}
                          {String.fromCharCode(97 + optionIndex) === result.selectedAnswer?.toLowerCase() &&
                            !result.isCorrect && <span className="text-red-500"> (Your Answer)</span>}
                          {String.fromCharCode(97 + optionIndex) === result.selectedAnswer?.toLowerCase() &&
                            result.isCorrect && <span className="text-blue-500"> (Your Answer)</span>}
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <button
                onClick={() => navigate('/results', { state: { submissionResult } })}
          ...
          <div className="space-y-6">
            ...
            <div>
              <button
                ...
                disabled={submissionLoading || quizCompleted || !quizStarted}
                className={`w-full px-6 py-3 rounded-lg font-semibold text-white shadow-md focus:outline-none focus-ring-2 focus-ring-indigo-500 focus-ring-offset-2 ${
                  submissionLoading || quizCompleted || !quizStarted
