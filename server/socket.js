...
export const setupSocket = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("sendMessage", async (data) => {
      const { groupId, content } = data;
      console.log(
        `[SOCKET] Received sendMessage event from user ${socket.user._id} (${socket.user.name})`
      );
      ...
      try {
        ...
        console.log(
          `[SOCKET] Querying ChatGroup for groupId: ${groupId} and member: ${socket.user._id}`
        );
        const chatGroup = await ChatGroup.findOne({
          _id: groupId,
          members: socket.user._id,
        ...
        if (!chatGroup) {
          console.log(
            `[SOCKET] Error: User ${socket.user._id} is not a member of group ${groupId}`
          );
        ...
        console.log(
          `[SOCKET] Populating sender info for message ID: ${message._id}`
        );
